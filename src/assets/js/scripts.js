SmartApp =  function (SmartApp, $, window, document) {
    "use strict";
	// Defined Variables
    var $win		= $(window), 
		$doc		= $(document),
	...
	// Return Check @v1.0
	SmartApp.Win = {};
	SmartApp.Win.height = $(window).height();
	SmartApp.Win.width = $(window).width();
	
	// getStatus @v1.0
	SmartApp.getStatus = {};
	SmartApp.getStatus.isRTL = ($body.hasClass('has-rtl') || $body.attr('dir') === 'rtl') ? true : false;
	SmartApp.getStatus.isTouch = (("ontouchstart" in document.documentElement)) ? true : false;
	SmartApp.getStatus.isMobile = (navigator.userAgent.match(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Windows Phone|/i)) ? true : false;
	SmartApp.getStatus.asMobile = (SmartApp.Win.width < _mobBreak) ? true : false;
	...
	SmartApp.Util = {};
	// ClassInit !Util @v1.0
	SmartApp.Util.classInit = function() {
		var hastouch = function () {
				if (SmartApp.getStatus.isTouch===true) { 
			...
			mobileview = function () {
				if (SmartApp.getStatus.asMobile===true) { 
    ...
    SmartApp.components.docReady.push(SmartApp.Util.classInit);
	
    // PreLoader !Util @v1.0
    SmartApp.Util.preLoader = function () {
	...
	SmartApp.components.winLoad.push(SmartApp.Util.preLoader);
	
	// BackTop !Util @v1.0
	SmartApp.Util.backTop = function () {
		var $backtop = $('.backtop');
	...
	SmartApp.components.docReady.push(SmartApp.Util.backTop);
    
    // Browser Check !Util @v1.0
    SmartApp.Util.browser = function() {
	...
	SmartApp.components.winLoad.push(SmartApp.Util.browser);
	
	// HeaderSticky !Util @v1.0
	SmartApp.Util.headerSticky = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Util.headerSticky);
    
	// imageBG !Util @v1.0
	SmartApp.Util.imageBG = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Util.imageBG);
	
	SmartApp.Util.Ovm = function () {
    ...
    SmartApp.components.docReady.push(SmartApp.Util.Ovm);

	// progressBar @v1.1
    SmartApp.Util.progressBar = function() {
        ...
        if($data_point.exists() && SmartApp.getStatus.isRTL === true){
            $data_point.each(function() {
	...
	SmartApp.components.docReady.push(SmartApp.Util.progressBar);
    
    // inputAnimation @v1.0
    SmartApp.Util.inputAnimate = function(){
	...
	SmartApp.components.docReady.push(SmartApp.Util.inputAnimate);
	
    // Dropdown @v1.0
    SmartApp.Util.toggler = function(){
	...
	SmartApp.components.docReady.push(SmartApp.Util.toggler);
	
	// accordionActive @v1.0
    SmartApp.Util.accordionActive = function() {
	...
	SmartApp.components.docReady.push(SmartApp.Util.accordionActive);
	
	// scrollAnimation !Util @v1.0
    SmartApp.Util.scrollAnimation = function () {
	...
	SmartApp.components.winLoad.push(SmartApp.Util.scrollAnimation);
    
	// Mainmenu/Nav @v1.0
	SmartApp.MainMenu = function() {
	...
	SmartApp.components.docReady.push(SmartApp.MainMenu);
	
	
    // OnePageScroll @v1.0
    SmartApp.OnePageScroll = function() {
	...
	SmartApp.components.docReady.push(SmartApp.OnePageScroll);

    //scrollAct @v1.0
    SmartApp.scrollAct = function() {
	...
	SmartApp.components.docReady.push(SmartApp.scrollAct);
	
	//ModalFix @v1.0
	SmartApp.modalFix = function() {
	   var $modal = $('.modal');
	...
	SmartApp.components.docReady.push(SmartApp.modalFix);
	...
	SmartApp.Plugins.countdown = function () {
		var $count = $('.countdown');
        if ($count.exists()) {
            $count.each(function() {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.countdown);
	
    
    // Carousel !Plugin @v1.1
    SmartApp.Plugins.carousel = function () {
    ...
    SmartApp.components.docReady.push(SmartApp.Plugins.carousel);
    
    // Flex Slider !Plugin @v1.9.1
	SmartApp.Plugins.flexslider = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.flexslider);
	
	// Select2 !Plugin @v1.0
	SmartApp.Plugins.select2 = function () {
		var $select = $(".select");
		if ($select.exists()) {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.select2);
    
    // Validator !Plugin @v1.0
	SmartApp.Plugins.validform = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.validform);

	// Form Validation !Plugin @v1.0
	SmartApp.Plugins.submitform = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.submitform);
    
	// Parallax !Plugin @v1.0
	SmartApp.Plugins.parallax = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.parallax);
	
    // Popup !Plugin @v1.0
	SmartApp.Plugins.popup = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.popup);
    
    // particlesJS !Plugin @v1.0
	SmartApp.Plugins.particles = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.particles);
    
    SmartApp.Plugins.videoBG = function () {
		...
		if($videobg.exists()) {
			$videobg.each(function(){
                ...
                if ( cover_value && SmartApp.Win.width < _mobBreak) {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.videoBG);
    $win.on('resize', function(){
        SmartApp.components.docReady.push(SmartApp.Plugins.videoBG);
    });
    
    // Data Tables @v100
    SmartApp.Plugins.dataTable = function () {
	...
	SmartApp.components.docReady.push(SmartApp.Plugins.dataTable);
	
	return SmartApp;
}(SmartApp, jQuery, window, document);
/* END @iO */